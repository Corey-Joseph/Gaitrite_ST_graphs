...
#Insert file path of Gaitrite spreadsheet
df = pd.read_excel('Z:\\Github_code\\Gaitrite_ST_graphs\\ELER 11April23 Initial Ax.xlsx')
#print(df)
...
#colors = ['green','orange','darkmagenta','darkcyan']
colors_fp = ['red', 'blue']
#colors_fp = ['darkmagenta', 'darkcyan']

velsd = [0.16, Both_Walking_Speed_SD, Left_Walking_Speed_SD, Right_Walking_Speed_SD]
...
ebc = [8.34, Both_Cadence_SD, Left_Cadence_SD, Right_Cadence_SD]
...
sldata = [0.71, round(Both_Step_Length,2), round(Left_Step_Length,2), round(Right_Step_Length,2)]
sldata_ = [0.71/2, Both_Step_Length/2, Left_Step_Length/2, Right_Step_Length/2]
ebsl = [0.06, Both_Step_Length_SD, Left_Step_Length_SD, Right_Step_Length_SD]
...
#here
stldata = [1.42, round(Both_Stride_Length,2), round(Left_Stride_Length,2), round(Right_Stride_Length,2)]
stldata_ =[1.42/2, Both_Stride_Length/2, Left_Stride_Length/2, Right_Stride_Length/2]
errstl = [0.12, Both_Stride_Length_SD, Left_Stride_Length_SD, Right_Stride_Length_SD]
...
ssdata = [0.41, round(Both_Single_Support,2), round(Left_Single_Support,2), round(Right_Single_Support,2)]
ssdata_ = [0.41/2, Both_Single_Support/2, Left_Single_Support/2, Right_Single_Support/2]
errss = [0.03, Both_Single_Support_SD, Left_Single_Support_SD, Right_Single_Support_SD]
...
errds = [0.05, Both_Double_Support_SD, Left_Double_Support_SD, Right_Double_Support_SD]
...
errsw = [0.04, Both_Step_Width_SD, Left_Step_Width_SD, Right_Step_Width_SD]
...
footprogdata = [round(Left_Foot_progression,2),round(Right_Foot_progression,2)]
#errfp = [5, 5, 5, 5]
axs[3, 1].set_title("Foot progression", fontsize=10)
axs[3, 1].tick_params(axis='x', labelsize=10)
axs[3, 1].tick_params(axis='y', labelsize=8)
axs[3, 1].set_ylabel('Angle (deg)', fontsize=8)
axs[3, 1].scatter(x_fp, footprogdata, color=colors_fp)
plt.yticks([-2,0,2], ['Internal', '0', 'External'])
for i, v in enumerate(footprogdata):
    axs[3, 1].text(i, footprogdata[i], footprogdata[i], fontsize=7, color="gray", fontweight='bold')
...
fig.suptitle("TS Graphs Average(SD) " + "ELER",  y=0.02, fontsize=8, color='gray')
